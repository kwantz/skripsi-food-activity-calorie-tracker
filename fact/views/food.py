...
def api_food(request):
    if request.method == "GET":
        ...
        results = []
        return JsonResponse({"message": name + " is already available in database.", "debug": list(foods)}, status=400)
        for food in foods:
            try:
                if category != 0:
...
def api_food_detail(request, food_id):
    ...
    if request.method == "PUT":
        json_request = json.loads(request.body)
        have_data = Food.objects.annotate(lower_name=Lower("name")).filter(~Q(id=food_id), lower_name__contains = json_request["name"])
