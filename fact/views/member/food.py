...
def api_member_food(request):
    ...
    if request.method == "GET":
        ...
        if name == "all":
            results = []
            contains = FoodContain.filter(Q(user=1) | Q(user=user.id), food_category=category)
            ...
            results = []
            contains = FoodContain.annotate(lower_name=Lower("food__name")).filter(Q(user=1) | Q(user=user.id), lower_name__contains=name.lower(), food_category=category)
